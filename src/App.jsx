...
// ── IRC WebSocket → badge-info sub months ──────────────────────────────────
...
// ── BANNER TIER ────────────────────────────────────────────────────────────
// Basé sur les mois réels de sub via IRC
function getBannerTier(subMonths, isSub) {
  if (!isSub || subMonths === null) return 0;   // Bronze
  if (subMonths < 3)  return 1;                  // Silver
  if (subMonths < 6)  return 2;                  // Gold
  if (subMonths < 12) return 3;                  // Platinum
  return 4;                                       // Diamond 12 mois+
}
...
const RANKS = [
  ...
  { name: "Silver",   stars: 2 },
  { name: "Gold",     stars: 3 },
  { name: "Platinum", stars: 4 },
  { name: "Diamond",  stars: 5 },
];

const bannerStyles = [
  {
    bg: "linear-gradient(135deg,#6B3A1F 0%,#C8956C 45%,#7B4A2F 100%)",
    border: "2px solid #A06030",
    shadow: "0 0 10px #5A3010aa,inset 0 1px 0 rgba(255,200,130,0.25)",
    textColor: "#FFE0A0", textShadow: "1px 2px 6px #3a2010,0 0 10px rgba(200,120,40,0.4)",
    starColor: "#FFD700", starGlow: "rgba(200,140,60,0.6)", shimmer: false,
    subColor: "#FFD070",
  },
  {
    bg: "linear-gradient(135deg,#6A7D8E 0%,#BDD0E0 45%,#7A8D9E 100%)",
    border: "2px solid #8AAABB",
    shadow: "0 0 14px rgba(160,200,230,0.3),inset 0 1px 0 rgba(255,255,255,0.3)",
    textColor: "#E8F4FF", textShadow: "1px 2px 4px #304050,0 0 12px rgba(150,200,255,0.5)",
    starColor: "#C8E0F8", starGlow: "rgba(150,200,240,0.6)", shimmer: false,
    subColor: "#A8D0F0",
  },
  {
    bg: "linear-gradient(135deg,#A07010 0%,#FFD700 40%,#FFA800 70%,#A07010 100%)",
    border: "2px solid #FFD700",
    shadow: "0 0 22px rgba(255,200,0,0.5),inset 0 1px 0 rgba(255,255,200,0.4)",
    textColor: "#FFF8DC", textShadow: "1px 2px 4px #6B4400,0 0 14px rgba(255,210,0,0.7)",
    starColor: "#FFFACD", starGlow: "rgba(255,210,0,0.7)", shimmer: false,
    subColor: "#FFE080",
  },
  {
    bg: "linear-gradient(135deg,#5A3A8C 0%,#C0A0F0 35%,#8060C0 65%,#6040A0 100%)",
    border: "2px solid #A080E0",
    shadow: "0 0 22px rgba(170,120,255,0.45),inset 0 1px 0 rgba(220,180,255,0.35)",
    textColor: "#F0E8FF", textShadow: "1px 2px 4px #3A2060,0 0 16px rgba(180,130,255,0.7)",
    starColor: "#DCC8FF", starGlow: "rgba(180,130,255,0.7)", shimmer: false,
    subColor: "#D0B0FF",
  },
  {
    bg: "linear-gradient(135deg,#050D1A 0%,#0E2550 35%,#081830 65%,#020810 100%)",
    border: "2px solid #3A9AFF",
    shadow: "0 0 28px rgba(50,150,255,0.65),0 0 55px rgba(50,150,255,0.2),inset 0 1px 0 rgba(100,180,255,0.2)",
    textColor: "#70D8FF", textShadow: "0 0 12px #38B0FF,0 0 24px #0070FF,1px 2px 6px #000",
    starColor: "#50D0FF", starGlow: "rgba(50,160,255,0.8)", shimmer: true,
    subColor: "#60C8FF",
  },
];
    ...
    <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
      ...
      <div style={{ display: "flex", gap: 3 }}>
        {Array.from({ length: r.stars }).map((_, i) => (
          <svg key={i} width={starSz} height={starSz} viewBox="0 0 24 24" fill={s.starColor}
            style={{ filter: `drop-shadow(0 0 3px ${s.starGlow})` }}>
      ...
      <div style={{
        ...
        background: s.bg, border: s.border, borderRadius: 9,
        ...
        {s.shimmer && (
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(105deg,transparent 38%,rgba(80,180,255,0.12) 50%,transparent 62%)", animation: "shimmer 2.5s infinite" }} />
        )}
        <span style={{
          color: s.textColor, fontFamily: "'Cinzel','Georgia',serif", fontWeight: 700,
          fontSize: fs, letterSpacing: "0.07em", textShadow: s.textShadow,
          zIndex: 1, padding: "0 10px", maxWidth: w - 20,
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
        }}>{username}</span>
      </div>

      {/* Durée sub sous la plaque */}
      {subDuration && (
        <div style={{
          ...
          <svg width="8" height="8" viewBox="0 0 24 24" fill={s.subColor}>
            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
          </svg>
          {subDuration}
        </div>
...
const Icon = {
  ...
  Shop:    () => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M19 6h-2c0-2.76-2.24-5-5-5S7 3.24 7 6H5c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm-7-3c1.66 0 3 1.34 3 3H9c0-1.66 1.34-3 3-3zm0 10c-1.66 0-3-1.34-3-3h2c0 .55.45 1 1 1s1-.45 1-1h2c0 1.66-1.34 3-3 3z"/></svg>,
  ...
  Clock:   () => <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg>,
};
...
const TABS = [
  ...
  { id: "shop",        label: "Boutique",    Icon: Icon.Shop },
];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
        ...
        <div style={{ background: "linear-gradient(135deg,rgba(145,71,255,0.12),rgba(60,20,120,0.2))", border: "1px solid rgba(145,71,255,0.25)", borderRadius: 12, padding: "18px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          ...
          <a href={`https://www.twitch.tv/products/${BROADCASTER}/ticket?ref=gift`} target="_blank" rel="noreferrer"
...
function ClipsPage({ token }) {
  ...
  const load = useCallback(async (after = null) => {
    ...
    try {
      const data = await twitchGet("/clips", token, params);
      setClips(data.data || []);
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 22, alignItems: "center" }}>
        ...
        {["week","month"].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            padding: "7px 18px", borderRadius: 20, border: "none", cursor: "pointer", fontWeight: 700, fontSize: "0.82rem",
            background: filter===f ? "linear-gradient(135deg,#9147ff,#6020c0)" : "rgba(255,255,255,0.06)",
            color: filter===f ? "#fff" : "#8080a0",
            boxShadow: filter===f ? "0 4px 14px rgba(145,71,255,0.35)" : "none", transition: "all 0.18s",
          }}>{f==="week" ? "Cette semaine" : "Ce mois"}</button>
        ))}
    ...
    <div>
      {!isFollower && userInfo && (
        <div style={{ background: "rgba(255,80,80,0.08)", border: "1px solid rgba(255,80,80,0.25)", borderRadius: 12, padding: "14px 20px", marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <a href="https://www.twitch.tv/daems_" target="_blank" rel="noreferrer"
            style={{ background: "linear-gradient(135deg,#e91916,#900)", color: "#fff", padding: "7px 18px", borderRadius: 20, fontWeight: 700, textDecoration: "none", animation: "pulse 1.5s infinite", fontSize: "0.82rem" }}>
            Suivre 💜
          </a>
        </div>
      )}
      {!isSub && userInfo && (
        <div style={{ background: "rgba(145,71,255,0.08)", border: "1px solid rgba(145,71,255,0.25)", borderRadius: 12, padding: "14px 20px", marginBottom: 22, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <a href={`https://www.twitch.tv/products/${BROADCASTER}/ticket`} target="_blank" rel="noreferrer"
            style={{ background: "linear-gradient(135deg,#9147ff,#5010b0)", color: "#fff", padding: "7px 18px", borderRadius: 20, fontWeight: 700, textDecoration: "none", animation: "pulse 1.5s infinite", fontSize: "0.82rem" }}>
            S'abonner ⭐
          </a>
        </div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 22 }}>
        <div>
          ...
          {loading ? <LoadSpinner /> : followers.length === 0 ? (
            <p style={{ color: "#505070", fontSize: "0.82rem" }}>Données non disponibles — réservé au broadcaster.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {followers.slice(0,30).map((f,i) => (
                <LeaderRow key={f.user_id} rank={i+1} name={f.user_name}
                  sub={new Date(f.followed_at).toLocaleDateString("fr-FR")}
                  highlight={userInfo?.id===f.user_id} accent="#9147ff" />
              ))}
            </div>
          )}
        </div>
        <div>
          ...
          {loading ? <LoadSpinner /> : subs.length === 0 ? (
            <p style={{ color: "#505070", fontSize: "0.82rem" }}>Données réservées au broadcaster.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {subs.slice(0,30).map((s,i) => (
                <LeaderRow key={s.user_id} rank={i+1} name={s.user_name}
                  sub={`Tier ${s.tier?.[0]||1}${s.is_gift?" · Gift":""}`}
                  highlight={userInfo?.id===s.user_id} accent="#f59e0b" />
              ))}
            </div>
          )}
        </div>
...
function DaemsLogo({ size = "md" }) {
  const fs = size === "lg" ? "1.9rem" : "1.35rem";
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: size==="lg"?"center":"flex-start", lineHeight: 1 }}>
      <span style={{ fontFamily: "'Cinzel','Georgia',serif", fontWeight: 900, fontSize: fs, background: "linear-gradient(135deg,#9147ff 0%,#bf94ff 55%,#ff79c6 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", letterSpacing: "0.05em", filter: "drop-shadow(0 0 14px rgba(145,71,255,0.45))" }}>
        DAEMS_
      </span>
      <span style={{ fontSize: "0.56rem", color: "#403560", letterSpacing: "0.22em", textTransform: "uppercase", paddingLeft: 2 }}>
        {size==="lg" ? "Official Fan Zone" : "Fan Zone"}
      </span>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    twitchGet("/users", token)
      .then(async d => {
        ...
        setUserInfo(u);
        try {
          const f = await twitchGet("/channels/followed", token, { user_id: u.id, broadcaster_id: BROADCASTER_ID });
          setIsFollower((f.data?.length ?? 0) > 0);
  ...
  return (
    <div style={appStyle}>
        ...
        <nav style={{ display: "flex", gap: 2 }}>
          {TABS.map(({ id, label, Icon: Ic }) => (
            <button key={id} onClick={() => setTab(id)} style={{
              padding: "7px 16px", borderRadius: 9, border: "none", cursor: "pointer", fontWeight: tab===id?700:500,
              fontSize: "0.82rem", background: tab===id?"rgba(145,71,255,0.2)":"transparent",
              color: tab===id?"#e0d0ff":"#706090",
          ...
          {userInfo && (
            <>
              ...
              <div style={{ display: "flex", gap: 5 }}>
                ...
                {isSub      && <Badge color="#9147ff"><Icon.Star />Sub {subMonths !== null ? `· ${subMonths}m` : ""}</Badge>}
              </div>
              <img src={userInfo.profile_image_url} alt="" style={{ width: 34, height: 34, borderRadius: "50%", border: "2px solid #9147ff44" }} />
            </>
          )}
          <button onClick={logout} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "#706090", padding: "6px 13px", cursor: "pointer", fontSize: "0.78rem", display: "flex", alignItems: "center", gap: 6, transition: "all 0.15s" }}
      ...
      <main style={{ paddingTop: 70, padding: "86px 28px 48px", maxWidth: 1280, margin: "0 auto", width: "100%" }}>
...
const headerStyle = {
  ...
  background: "rgba(14,14,16,0.94)", backdropFilter: "blur(18px)",
  borderBottom: "1px solid rgba(145,71,255,0.15)",
  display: "flex", alignItems: "center", justifyContent: "space-between",
  padding: "0 24px", boxShadow: "0 4px 24px rgba(0,0,0,0.4)",
};
